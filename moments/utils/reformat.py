...
def compact_reformat(source_dir, target_dir, all_files=False, max_dumps=None):
    ...
    #This is checking if the source directory is a string or not
    ...
    #this is checking the target dir
    target_dir = os.path.abspath(target_dir) + "/"
    
    #making templates for the directories
    ...
    #initializing the RBO variables
    ...
    #creating a map between the file types and their settings in RBO
    ...
    #setting a path to find bob2hv
    bob2hv_path = os.path.abspath(pkg_resources.resource_filename("moments.utils", "/bin/bob2hv"))
    
    #making sure that the algorithm treats the hires data differently
    is_hires = ["FV-hires"]
    
    #mapping between the bob2hv variables and the file names same as above
    ...
    #calling the function get_profile_types
    #source is a PPMdir object that holds the information about the data directory
    profiles = source.get_profile_types()
    print source.get_bobfile_types()
    for ftype in source.get_bobfile_types():
        if "FVandMoms" in ftype:
            bobfiles.append(ftype)#generating a list of bobfile types in the directory
        else:
            hvfiles.append(ftype)#trying to sort out which are bob and which are hv?
    ppminfiles = source.get_ppminfile_types()#once again generating a list of the filetypes
    
    for file in source.get_source_code() + source.get_compile_script() + source.get_jobscript() + source.get_other_files():
        # now looking at all of the files and copying to the output directory
        fname = file.replace(source_dir, "")
        _copy_file(file, target_dir + fname)
    
    for ftype in profiles:
        # now trying to reformat the output using the format templates 
        ...
        for i, dump in enumerate(source.get_dumps(ftype)):
            ...
            for file in source.get_dumpfiles(ftype, dump):
                ...
                if RBO_in_ftype is not None:
                    ...
                    print bytes,nbytes_theoretical
                    '''
        ...
        for RBO_key in RBO_input_map.keys():
            if ftype in RBO_key:
                ...
                for i, dump in enumerate(source.get_dumps(ftype)):
                        ...
                        #except:
                        #   print 'Warning: this directory may have the wrong format for conversion to .hv'

    # remove all temperary files
    shutil.rmtree(os.path.dirname(hv_source_format.format(ftype="")))
    '''
    def truncate_file(in_path,nbytes_theoretical):

        nteams = source_code_definitions["nnxteams"]*source_code_definitions["nnyteams"]*source_code_definitions["nnzteams"]
        filesize = nbytes_theoretical/nteams

        for file in os.listdir(in_path):
            truncate_command = ['tail','-c',file_size,os.path.join(in_path,file)>out_path]
            subprocess.call(truncate_command,
    '''
